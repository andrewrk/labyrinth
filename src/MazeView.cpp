...
void MazeView::display() {
    ...
    // rectangle from m_pos to m_pos+m_size
    glColor3f(0.1, 0.1, 0.9);
    glBegin(GL_POLYGON);
        glVertex3f(m_pos.x, m_pos.y, m_pos.z);
        glVertex3f(m_pos.x, m_pos.y+m_size.y, m_pos.z);
        glVertex3f(m_pos.x+m_size.x, m_pos.y+m_size.y, m_pos.z);
        glVertex3f(m_pos.x+m_size.x, m_pos.y, m_pos.z);
    glEnd();
